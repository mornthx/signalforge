...
def gen_qss(theme: dict[str, Any], version: str, theme_name: str) -> str:
    ...
    lines.append("}")
    lines.append("")
    lines.append("QMenuBar {")
    lines.append(f"    background-color: {color['bg.primary']};")
    lines.append(f"    color: {color['text.primary']};")
    lines.append(f"    border-bottom: 1px solid {color['border']};")
    lines.append(f"    padding: 0 {spacing['xs']}px;")
    lines.append("}")
    lines.append("QMenuBar::item {")
    ...
    lines.append(f"    padding: {spacing['xs']}px;")
    lines.append("}")
    lines.append("QMenu::item {")
    lines.append(f"    padding: {spacing['xs']}px {spacing['lg']}px {spacing['xs']}px {spacing['sm']}px;")
    lines.append("}")
    lines.append("")
    lines.append("QToolBar {")
    lines.append(f"    background-color: {color['bg.primary']};")
    lines.append(f"    border: none;")
    lines.append(f"    border-bottom: 1px solid {color['border']};")
    lines.append(f"    spacing: {spacing['xs']}px;")
    lines.append(f"    padding: {spacing['xs']}px;")
    lines.append("}")
    lines.append("QToolBar::separator {")
    lines.append(f"    background-color: {color['border']};")
    lines.append(f"    width: 1px;")
    lines.append(f"    margin: {spacing['xs']}px;")
    lines.append("}")
    lines.append("")
    lines.append("QDockWidget {")
    lines.append(f"    background-color: {color['bg.primary']};")
    lines.append(f"    color: {color['text.primary']};")
    lines.append(f"    titlebar-close-icon: none;")
    lines.append(f"    titlebar-normal-icon: none;")
    lines.append("}")
    lines.append("QDockWidget::title {")
    lines.append(f"    background-color: {color['bg.primary']};")
    lines.append(f"    color: {color['text.primary']};")
    lines.append(f"    border-bottom: 1px solid {color['border']};")
    lines.append(f"    padding: {spacing['xs']}px {spacing['sm']}px;")
    ...
    lines.append("}")
    lines.append("QSplitter::handle {")
    lines.append(f"    background-color: {color['border']};")
    lines.append("}")
    lines.append("QSplitter::handle:hover {")
    lines.append(f"    background-color: {color['border.focus']};")
    lines.append("}")
    lines.append("")
    lines.append("/* ===== Controls ===== */")
    lines.append("QPushButton, QToolButton {")
    lines.append(f"    background-color: {color['bg.elevated']};")
    lines.append(f"    color: {color['text.primary']};")
    lines.append(f"    border: 1px solid {color['border']};")
    lines.append(f"    padding: {spacing['xs']}px {spacing['sm']}px;")
    lines.append("}")
    lines.append("QPushButton:hover, QToolButton:hover {")
    lines.append(f"    border-color: {color['border.focus']};")
    lines.append("}")
    lines.append("QPushButton:pressed, QToolButton:pressed {")
    lines.append(f"    background-color: {color['border']};")
    lines.append("}")
    lines.append("QPushButton:focus, QToolButton:focus {")
    lines.append(f"    border: 2px solid {color['border.focus']};")
    lines.append("}")
    lines.append("QPushButton:disabled, QToolButton:disabled {")
    lines.append(f"    color: {color['text.disabled']};")
    lines.append("}")
    lines.append("QPushButton[class=\"primary\"] {")
    lines.append(f"    background-color: {color['border.focus']};")
    lines.append(f"    color: {color['bg.primary']};")
    lines.append(f"    border: 1px solid {color['border.focus']};")
    lines.append("}")
    lines.append("QPushButton[class=\"primary\"]:hover {")
    lines.append(f"    border: 2px solid {color['text.primary']};")
    lines.append("}")
    lines.append("")
    ...
    lines.append(f"    color: {color['bg.primary']};")
    lines.append("}")
    lines.append("QListWidget, QTreeView {")
    lines.append(f"    background-color: {color['bg.surface']};")
    lines.append(f"    color: {color['text.primary']};")
    lines.append(f"    border: 1px solid {color['border']};")
    lines.append("}")
    lines.append("QListWidget::item, QTreeView::item {")
    lines.append(f"    padding: {spacing['xs']}px {spacing['sm']}px;")
    lines.append("}")
    lines.append("QListWidget::item:selected, QTreeView::item:selected {")
    lines.append(f"    background-color: {color['border.focus']};")
    lines.append(f"    color: {color['bg.primary']};")
    lines.append("}")
    lines.append("")
    lines.append("/* ===== Panel chrome ===== */")
    lines.append("QFrame#chartFrame, QFrame#chartEmptyState {")
    lines.append(f"    background-color: {color['bg.surface']};")
    lines.append(f"    border: 1px solid {color['border']};")
    lines.append("}")
    lines.append("QFrame#statusStrip {")
    lines.append(f"    background-color: {color['bg.elevated']};")
    lines.append("}")
    lines.append("QFrame#statusCell {")
    lines.append(f"    border-left: 1px solid {color['border']};")
    lines.append("}")
    ...
    lines.append("")
    lines.append("/* ===== Workbench frame (M34 redesign) ===== */")
    lines.append("/* Activity rail, segmented context switch, inspector, drawer, status")
    lines.append("   chips, empty state. objectName-scoped so they only style the new")
    lines.append("   frame — see src/workbench/. */")
    lines.append("QFrame#workbenchTopBar {")
    lines.append(f"    background-color: {color['bg.primary']};")
    lines.append(f"    border-bottom: 1px solid {color['border']};")
    lines.append("}")
    lines.append("QFrame#activityRail {")
    lines.append(f"    background-color: {color['bg.elevated']};")
    lines.append(f"    border-right: 1px solid {color['border']};")
    lines.append("}")
    lines.append("QToolButton#railButton {")
    lines.append("    background-color: transparent;")
    lines.append(f"    color: {color['text.secondary']};")
    lines.append("    border: none;")
    lines.append("    border-left: 2px solid transparent;")
    lines.append(f"    padding: {spacing['sm']}px {spacing['xs']}px;")
    lines.append("}")
    lines.append("QToolButton#railButton:hover {")
    lines.append(f"    color: {color['text.primary']};")
    lines.append(f"    background-color: {color['bg.surface']};")
    lines.append("}")
    lines.append("QToolButton#railButton:checked {")
    lines.append(f"    color: {color['accent']};")
    lines.append(f"    border-left: 2px solid {color['accent']};")
    lines.append("}")
    lines.append("QToolButton#railButton:focus {")
    lines.append(f"    border-left: 2px solid {color['border.focus']};")
    lines.append("}")
    lines.append("QToolButton#segmentButton {")
    lines.append("    background-color: transparent;")
    lines.append(f"    color: {color['text.secondary']};")
    lines.append("    border: none;")
    ...
    lines.append(f"    padding: {spacing['xs']}px {spacing['sm']}px;")
    lines.append("}")
    lines.append("QToolButton#segmentButton:hover {")
    lines.append(f"    color: {color['text.primary']};")
    lines.append("}")
    lines.append("QToolButton#segmentButton:checked {")
    lines.append(f"    color: {color['accent']};")
    lines.append(f"    border-bottom: 2px solid {color['accent']};")
    lines.append("}")
    lines.append("QFrame#workbenchInspector {")
    lines.append(f"    background-color: {color['bg.elevated']};")
    lines.append(f"    border-left: 1px solid {color['border']};")
    lines.append("}")
    lines.append("QFrame#workbenchDrawer {")
    lines.append(f"    background-color: {color['bg.elevated']};")
    lines.append(f"    border-top: 1px solid {color['border']};")
    lines.append("}")
    lines.append("QFrame#statusChip {")
    lines.append(f"    background-color: {color['bg.elevated']};")
    lines.append(f"    border: 1px solid {color['border']};")
    lines.append("    border-radius: 8px;")
    lines.append("}")
    lines.append("QFrame#statusChip QLabel {")
    lines.append(f"    color: {color['text.secondary']};")
    lines.append("}")
    lines.append("QFrame#statusChip[state=\"connected\"] QLabel,")
    lines.append("QFrame#statusChip[state=\"good\"] QLabel,")
    lines.append("QFrame#statusChip[state=\"live\"] QLabel {")
    lines.append(f"    color: {color['status.connected']};")
    lines.append("}")
    lines.append("QFrame#statusChip[state=\"connecting\"] QLabel,")
    lines.append("QFrame#statusChip[state=\"disconnecting\"] QLabel,")
    lines.append("QFrame#statusChip[state=\"paused\"] QLabel,")
    lines.append("QFrame#statusChip[state=\"stale\"] QLabel,")
    lines.append("QFrame#statusChip[state=\"uncertain\"] QLabel {")
    lines.append(f"    color: {color['status.connecting']};")
    lines.append("}")
    lines.append("QFrame#statusChip[state=\"error\"] QLabel,")
    lines.append("QFrame#statusChip[state=\"bad\"] QLabel,")
    lines.append("QFrame#statusChip[state=\"recording\"] QLabel {")
    lines.append(f"    color: {color['status.error']};")
    lines.append("}")
    lines.append("QFrame#statusChip[state=\"replay\"] QLabel {")
    lines.append(f"    color: {color['mode.replay']};")
    lines.append("}")
    lines.append("QFrame#emptyState {")
    lines.append(f"    background-color: {color['bg.surface']};")
    lines.append("}")
    lines.append("")
