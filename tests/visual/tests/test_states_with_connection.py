...
def test_connected_state_matches_baseline():
    """M16 cross-environment contract: `compare_with_contract` with
    env-sidecar enforcement + auto-discovered `.mask.json` (state
    04 ships a status-bar live-counter mask per S6.6)."""
    actual = _ensure_capture()
