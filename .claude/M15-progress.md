...
| S6 | CC autonomy demonstration | done | (this commit) | `docs/m15-cc-autonomy-demo.md` walks through a recorded demo: Quit→Exit one-line regression at `main_window.cpp:1086` introduced; `test_baseline_30_menu_file_open` PASSes despite the regression (0.019 % diff << 5 % gate); CC Read tool catches the semantic regression; CC proposes the fix; fix applied + verified (0.0000 % post-fix). Demonstrates the V0.2 charter §1 perception loop AND the R7 operationalised lesson "pixel-diff necessary but not sufficient — vision-LLM closes the semantic gap". Single-commit reversible; reproduction recipe included. |
| S7 | M15-done.md + V0.3 hand-off | done (local) | (this commit) | `.claude/M15-done.md` (M15 closure report with §13 hand-off recipe), `docs/v0.2-vision-infrastructure.md` (V0.2 vision-loop usage guide), `tests/visual/README.md` (extended with R7 production-fidelity criterion). PR-ready content; commits queued locally until §8.1 cross-environment baseline drift resolved (operator-driven per-state acceptance of CI captures staged at `tests/screenshots/baseline-candidate/`). |
